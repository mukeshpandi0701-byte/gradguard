...
const StaffCalendar = () => {
  ...
  const holidayDates = events.filter(e => e.event_type === "holiday" && !isSunday(new Date(e.event_date))).map(e => new Date(e.event_date));
  const customSessionDates = events.filter(e => e.event_type === "custom_sessions").map(e => new Date(e.event_date));
