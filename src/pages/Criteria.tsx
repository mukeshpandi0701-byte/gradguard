  ...
  const checkRoleAndFetchCriteria = async () => {
    ...
    try {
      ...
      if (userIsHOD) {
        ...
        const { data: criteriaData, error: rpcError } = await supabase.rpc('get_department_hod_criteria');

        if (rpcError) {
          console.error("Error fetching criteria via RPC:", rpcError);
          setHodCriteriaExists(false);
        } else if (criteriaData && criteriaData.length > 0) {
          const c = criteriaData[0];
          
          if (c.hod_found) {
            setHodName(c.hod_name || "HOD");
          }
          ...
          if (c.criteria_found) {
            ...
            setCriteria({
              ...
              max_sessions_per_day: c.max_sessions_per_day ?? 7,
              ...
              assignment_weightage: c.assignment_weightage ?? 0,
            });
          } else {
            setHodCriteriaExists(false);
          }
